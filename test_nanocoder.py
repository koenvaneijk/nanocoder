...
class TestFormatTable(unittest.TestCase):
    def test_simple_table(self):
        ...
        result = nanocoder.format_table(lines)
    ...
    def test_single_line_passthrough(self):
        lines = ["just one line"]
        result = nanocoder.format_table(lines)
        self.assertEqual(result, "just one line")

class TestIsTableHelpers(unittest.TestCase):
    def test_is_table_sep(self):
        self.assertTrue(nanocoder.is_table_sep("| --- | --- |"))
        self.assertTrue(nanocoder.is_table_sep("|---|---|"))
        self.assertTrue(nanocoder.is_table_sep("| :---: | ---: |"))
        self.assertFalse(nanocoder.is_table_sep("| Name | Age |"))
        self.assertFalse(nanocoder.is_table_sep("regular text"))
    
    def test_is_table_row(self):
        self.assertTrue(nanocoder.is_table_row("| Name | Age |"))
        self.assertFalse(nanocoder.is_table_row("| --- | --- |"))
        self.assertFalse(nanocoder.is_table_row("no pipes here"))
...
class TestGetMapEdgeCases(unittest.TestCase):
    ...
    def test_get_map_no_definitions(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = nanocoder.get_map(tmpdir)
            # File should not appear since it has no function/class definitions
            self.assertNotIn("empty.py", result)
...
class TestFormatTableEdgeCases(unittest.TestCase):
...
class TestMain(unittest.TestCase):
    ...
    @patch('nanocoder.stream_chat')
    @patch('builtins.input')
    def test_update_command_no_change(self, mock_input, mock_stream):
        mock_input.side_effect = ["/update", EOFError, "/exit", EOFError]
        # Mock urlopen to return same content
        current = Path(nanocoder.__file__).read_text()
        mock_response = MagicMock()
        mock_response.read.return_value = current.encode()
        mock_response.__enter__ = MagicMock(return_value=mock_response)
        mock_response.__exit__ = MagicMock(return_value=False)
        with patch('builtins.print'):
    ...
    @patch('nanocoder.stream_chat')
    @patch('builtins.input')
    def test_update_command_failure(self, mock_input, mock_stream):
        mock_input.side_effect = ["/update", EOFError, "/exit", EOFError]
        with patch('builtins.print'):
...
class TestStreamChatSpinner(unittest.TestCase):
    ...
    @patch('urllib.request.urlopen')
    def test_table_buffering(self, mock_urlopen):
        mock_response = MagicMock()
        mock_response.__enter__ = MagicMock(return_value=mock_response)
        mock_response.__exit__ = MagicMock(return_value=False)
        
        # Stream a table - should be buffered
        content = "| A | B |\n| --- | --- |\n| 1 | 2 |\n\nAfter table"
        chunks = [b'data: ' + json.dumps({"choices": [{"delta": {"content": content}}]}).encode() + b'\n']
        mock_response.__iter__ = MagicMock(return_value=iter(chunks))
        mock_urlopen.return_value = mock_response
...
class TestStreamChatTryFlush(unittest.TestCase):
    ...
    @patch('urllib.request.urlopen')
    def test_table_buffering_prevents_flush(self, mock_urlopen):
        """Test that tables in recent lines prevent flushing"""
        mock_response = MagicMock()
        mock_response.__enter__ = MagicMock(return_value=mock_response)
        mock_response.__exit__ = MagicMock(return_value=False)
        
        # Content with table that shouldn't be flushed mid-table
        content = "| A | B |\n| --- | --- |\n| 1 | 2 |\n\nAfter"
        chunks = [b'data: ' + json.dumps({"choices": [{"delta": {"content": content}}]}).encode() + b'\n']
        mock_response.__iter__ = MagicMock(return_value=iter(chunks))
        mock_urlopen.return_value = mock_response
...
class TestFormatTableSingleLine(unittest.TestCase):
    """Test format_table with single line returns the line"""
    
    def test_single_line(self):
        result = nanocoder.format_table(["just one line"])
        self.assertEqual(result, "just one line")
...
class TestRenderMdProcessTables(unittest.TestCase):
    """Test the process_tables inner function in render_md"""
    ...
    def test_table_in_middle_of_text(self):
        ...
        self.assertIn("After", result)
        self.assertIn("┌", result)  # Table border
