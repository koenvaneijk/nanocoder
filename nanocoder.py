...
def format_table(lines):
    ...
    col_widths = [max(3, max((len(rows[i][j]) if j < len(rows[i]) else 0) for i in range(len(rows)) if i != 1)) for j in range(col_count)]
    s, v = '90m', f"{ansi('90m')}│{ansi('0m')}"
    border = lambda l,m,r: f"{ansi(s)}{l}" + m.join('─'*(w+2) for w in col_widths) + f"{r}{ansi('0m')}"
    result = [border('┌','┬','┐')]
    for i, row in enumerate(rows):
        row.extend([''] * (col_count - len(row)))
        if i == 1: result.append(border('├','┼','┤'))
        else: result.append(v + v.join((f"{ansi('1m')} {c.ljust(col_widths[j])} {ansi('22m')}" if i==0 else f" {c.ljust(col_widths[j])} ") for j,c in enumerate(row)) + v)
    return '\n'.join(result + [border('└','┴','┘')])
...
def is_table_sep(line): return bool(re.match(r'^\|?[\s]*:?-+:?[\s]*(\|[\s]*:?-+:?[\s]*)+\|?$', line.strip()))
def is_table_row(line): return '|' in line and line.strip() and not is_table_sep(line)
...
def stream_chat(messages, model):
    ...
    except urllib.error.HTTPError as e: stop_event.set(); spinner.join(); print(f"\n{styled(f'HTTP {e.code}: {e.reason}', '31m')}")
